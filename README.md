# CORTEX — The Shared Brain For AI Development

> **Git syncs code. CORTEX syncs understanding.**

CORTEX is a local-first AI development companion. It scans your GitHub repositories, builds a knowledge graph of each project (tech stack, authentication, features, structure), generates a tailored **system prompt** per project, and exposes everything to your AI coding tools over the **Model Context Protocol (MCP)** — so every agent, in every IDE, starts with full project context and shared memory.

The whole system runs locally as a single Go daemon (`cortexd`) built on [PocketBase](https://pocketbase.io), with a [SolidJS](https://www.solidjs.com) web UI.
...
1. **Scan GitHub repos** — Sign in with GitHub, then scan any repo. CORTEX clones it, indexes files/symbols, and analyzes the **tech stack, authentication approach, features, project structure, and API endpoints**, persisting a **knowledge graph** into local storage.
2. **Generate a system prompt** — For any scanned project, generate a project-specific agent "characterization" using Mistral or Ollama (with a heuristic fallback when no LLM is configured).
3. **Connect your AI tools via MCP** — Create a per-IDE authorized connection. The IDE loads the project characterization plus the memory of previous AI sessions, and writes new memory back as it works — recognized by which IDE/client produced it.
4. **Shared, persistent memory** — Whenever you start working on the same project again (in any tool), the full accumulated memory is available.
...
└────────────┬──────────────┘         │  Claude · Gemini · ...    │
             │ PocketBase REST          └────────────┬─────────────┘
             │ + ConnectRPC + /api/cortex             │ MCP (JSON-RPC / HTTP)
             ▼                                         ▼
┌──────────────────────────────────────────────────────────────┐
│                      cortexd  (Go daemon)                      │
│                                                                │
...
│  ├─ /api/cortex/*         (scan, providers, prompt, mcp admin) │
│  └─ /mcp                  (MCP server, token-authorized)       │
│                                                                │
│  Subsystems:                                                   │
│   Scanner · Analyzer · Git Sync · Search · Vector/Embeddings   │
│   LLM (Mistral / Ollama) · File Watcher · MCP Server           │
└──────────────────────────────────────────────────────────────┘
```
...
│   │
│   ├── auth/
│   │   └── github.go               # GitHub OAuth hooks + REST client (list/clone repos)
│   │
│   ├── config/
│   │   └── config.go               # cortex.yaml + env config loading
│   │
│   ├── daemon/
│   │   └── daemon.go               # Wires PocketBase, RPC, /api/cortex, /mcp, watcher
│   │
│   ├── db/
...
│   │
│   ├── git/
│   │   └── sync.go                 # Clone / commit / pull, .cortex/ directory mgmt
│   │
│   ├── llm/
...
│   │
│   ├── memory/
│   │   └── vault.go                # Export vault entries to .cortex/ markdown files
│   │
│   ├── rpc/                        # ConnectRPC service handlers
│   │   ├── server.go               #   Handlers struct + service mounting
│   │   ├── auth.go                 #   Bearer-token interceptor
│   │   ├── convert.go              #   Record → proto conversion
...
│   │
│   ├── search/
│   │   └── engine.go               # Keyword (FTS) search over collections
│   │
│   ├── vector/                     # Semantic-search abstraction (optional)
│   │   ├── store.go                #   Store + Embedder interfaces
│   │   ├── ollama.go               #   Ollama embedder
│   │   └── lancedb.go              #   LanceDB HTTP sidecar client
│   │
│   └── watcher/
│       └── watcher.go              # fsnotify-based incremental re-scans
│
...
│   │   ├── pages/
│   │   │   ├── Dashboard/  Projects/  Vaults/  Tasks/  Handoffs/
│   │   │   ├── Search/  Graph/  AIContext/  (AI Agents / prompt builder)
│   │   │   ├── MCPServer/          # MCP connections + per-platform integration guides
│   │   │   │   ├── MCPServer.tsx
│   │   │   │   └── integrations.ts # Per-client config catalog (Cursor, VS Code, CLIs...)
│   │   │   ├── Settings/  Login/
│   │   ├── components/             # Sidebar, TopBar, cards, feeds, etc.
│   │   ├── layouts/  App.tsx  index.tsx
│   │   └── index.css
│   └── package.json
│
├── cortex.yaml                     # Daemon configuration
├── buf.gen.yaml / buf.gen.ts.yaml  # Protobuf codegen config
...
Configuration is read from `cortex.yaml` (searched in `.`, `.cortex/`, and `~/.cortex/`) and overridden by `CORTEX_*` environment variables.
...
> **Settings → AI Agents**, and stored on the user record — not in `cortex.yaml`.
...
2. **Projects** page → your repos appear. Click **Scan** on a repo. CORTEX clones it, indexes it, and builds its knowledge graph (view it on the **Graph** page).
3. *(Optional)* **Settings → AI Agents** → add a **Mistral key** or point at **Ollama** for richer output and memory embeddings.
4. **AI Agents** page → pick a scanned project → **Generate System Prompt**. This is the project's "characterization."
5. **MCP Server** page → **New Connection** → pick the project + your AI client → copy the generated, client-specific config into your IDE/CLI.
6. Work as usual. Your AI calls `cortex_get_context` to load the characterization + prior memory, and `cortex_save_memory` to record progress/decisions. Next session — in any tool — picks up where you left off.
...
**Tools:** `cortex_get_context`, `cortex_save_memory`, `cortex_list_memories`, `cortex_get_tasks`
...
The **MCP Server** page generates the correct config for each client:

- **AI Agent CLIs:** Claude Code, Codex, Gemini CLI, GitHub Copilot, OpenCode, Factory
- **Web Clients:** Claude.ai, ChatGPT, Goose *(require a public HTTPS tunnel — localhost isn't reachable)*
- **IDEs:** Cursor, VS Code, Antigravity, Kiro, Windsurf
...
> CORTEX is local-first: your code, memory, and tokens stay on your machine in `~/.cortex/`.
